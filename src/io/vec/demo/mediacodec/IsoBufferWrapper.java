...
/**
 * A <code>FilterInputStream</code> enriched with helper methods to ease writing
 * of Iso specific numbers and strings.
 */
public class IsoBufferWrapper {
    ...
    public IsoBufferWrapper(byte[] bytes) {
    ...
    public int read(byte[] b, int off, int len) {
		...
		if (parents[activeParent].remaining() >= len) {
            ...
            activeParent++;
			if (parents.length > activeParent) {
				parents[activeParent].rewind();
    ...
    public IsoBufferWrapper getSegment(long startPos, long length) {
        ...
        while (length > 0) {
            ...
            if (currentSlice.remaining() >= length) {
                currentSlice.limit((int) length); // thats ok we tested in the
                                                  // line before
                length -= length;
            } else {
        ...
        position(savePos);
        return new IsoBufferWrapper(segments.toArray(new ByteBuffer[segments.size()]));
    }
